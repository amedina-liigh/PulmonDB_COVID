...
subtitle: "Regulons Selection: tissues"
date: "`r Sys.Date()`"
...
params:
  ...
  metadata.fn : ~/Documents/RegGenoLab/COVID19/pyscenic/regulons-analysis/out/organs-desgtex/desai2020_gtexv8-subTissues-sample_source_disease.csv # experiment metadata
  ...
  tfs_DA_2t.fn : tfs_mwu_ks_fdr0005-0295-2t.RData
  tfs_DA_enrichLFC_2t.fn : tfs_DA0005-0295_enrichLFC_2t.RData
...
params$lung <- 0
# dirs
params$inputdir <- "/home/user/Documents/RegGenoLab/COVID19/pyscenic/regulons-analysis/out/organs-desgtex2/"
params$outdir <- "/home/user/Documents/RegGenoLab/COVID19/pyscenic/regulons-analysis/out/organs-desgtex2/plots/"
# files
params$TF_Family_DBD_hsapiens_db <- "~/Documents/RegGenoLab/dbs/TF_Family_DBD-hsapiens.csv"
params$metadata.fn <- "~/Documents/RegGenoLab/COVID19/pyscenic/regulons-analysis/out/organs-desgtex/desai2020_gtexv8-subTissues-sample_source_disease.csv"
# direct out
params$tfs.fn <- "tfs947-ordered.txt" # tfs guiding regs- after filtering
...
params$tfs_DA_enrichLFC_2t.fn <- "tfs_DA0005-0295_enrichLFC_2t.RData" #tfs_DA24_enrichLFC_2t.RData
params$tfs_DA_2t.fn <- "tfs_mwu_ks_fdr0005-0295-2t.RData" # tfs_mwu_ks_fdr24-2t.RData
# tests pvalues
...
```{r, message=FALSE}
thrs.mwu <- regsPerFDRthr(pv.adj.mwu, fdr_lowth = 0.0005, step = 0.001, fdr_upthr = 0.03)
thrs.mwu

# ALSO TRY
# thrs.mwu <- regsPerFDRthr(pv.adj.mwu, fdr_lowth = 0.05)
# thrs.mwu
```
...
```{r, message=FALSE}
fdrs.mwu <- getChosenFDRth(thrs.mwu, min.regs = c(100,93))
fdrs.mwu
...
```{r, message=FALSE}
thrs.klm <- regsPerFDRthr(pv.adj.klm, fdr_lowth = 0.0005, step = 0.001, fdr_upthr = 0.03)
thrs.klm
```
...
# tfs_ks_fdr
length(unique(c(
  tfs_mwu_fdr$Bowel, tfs_mwu_fdr$Heart, tfs_mwu_fdr$Kidney, 
  tfs_mwu_fdr$Liver, tfs_mwu_fdr$Lung
)))

length(unique(c(
  tfs_ks_fdr$Bowel, tfs_ks_fdr$Heart, tfs_ks_fdr$Kidney, 
  tfs_ks_fdr$Liver, tfs_ks_fdr$Lung
)))
...
length(unique(c(
  tfs_da$Bowel, tfs_da$Heart, tfs_da$Kidney, 
  tfs_da$Liver, tfs_da$Lung
)))
```
...
## number of regulons in DA
length(unique(c(
  tfs_da_enrich$Bowel, tfs_da_enrich$Heart, tfs_da_enrich$Kidney, 
  tfs_da_enrich$Liver, tfs_da_enrich$Lung
)))
```
...
## number of regulons in DA
length(unique(c(
  tfs_da_enrich_up$Bowel, tfs_da_enrich_up$Heart, tfs_da_enrich_up$Kidney, 
  tfs_da_enrich_up$Liver, tfs_da_enrich_up$Lung
)))
...
List of all Differentially Activated **Dowregulated** Regulons:
